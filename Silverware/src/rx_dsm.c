...
if ( framestarted == 1){
      ...
      // TAER channel order
	#ifdef RX_DSMX_2048																												
	      rx[0] = (channels[3] - 1024.0f) * dsmx_scalefactor;
        rx[1] = (channels[0] - 1024.0f) * dsmx_scalefactor;
        rx[2] = -1*(channels[1] - 1024.0f) * dsmx_scalefactor;
        rx[3] =-1*((channels[2] - 1024.0f) * dsmx_scalefactor * 0.5f) + 0.5f;
